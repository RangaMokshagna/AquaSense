...
import {
  getReadings, getAllReadings, getPredictions, getAlerts,
...
export default function App() {
  ...
  // IoT/Simulator only — Live Dashboard
  const [sensorReadings, setSensorReadings] = useState([])
  const [sensorPreds,    setSensorPreds]    = useState([])

  // All readings including manual — History tab
  ...
  const fetchInitial = useCallback(async () => {
    try {
      ...
      const [iotData, allData, pData, aData, dData] = await Promise.all([
        getReadings(params),        // IoT only (backend filters source)
        getAllReadings(params),      // All including manual
        getPredictions(params),
        getAlerts({ ...params, resolved: false }),
        getDevices(),
      ])

      const iotReadings = iotData.data   || []
      const allRdgs     = allData.data   || []
      const predictions = pData.data     || []

      // Live Dashboard — IoT only
  ...
  // ── Live WebSocket — IoT only (backend already blocks manual) ─
  useEffect(() => {
    if (!liveReading) return
    const { reading, prediction } = liveReading

    // Extra safety — skip if source is manual
    const src = reading?.source || 'http'
    if (src === 'manual' || src === 'dataset-upload') return
  ...
  // ── Manual input — History + Alerts only, never Live Dashboard ─
  const handleManualReading = ({ reading, prediction, alerts: newAlerts }) => {
    ...
    // sensorReadings and chartData are NOT touched
  }
  ...
  return (
    <div className={styles.app}>
      ...
      <main className={styles.main}>
        ...
        {/* ── LIVE DASHBOARD — IoT sensors only ─────────── */}
        {activeTab === 'dashboard' && (
            ...
            {sensorReadings.length === 0 ? (
              <div className={styles.noData}>
                ...
                <p className={styles.noDataHint}>
                  Start the simulator: <code>node scripts\simulate.js</code>
